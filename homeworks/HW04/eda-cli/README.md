...
### 5. `POST /head` — вывод первых n строк датасета

Эндпоинт принимает CSV-файл и возвращает первые `n` строк датасета.
...
- возвращает первые n строк (df.head(n));
...
```json
{
  ...
  "latency_ms": 1.47,
  "dataset_shape": { "n_rows": 10000, "n_cols": 12 }
}
```
