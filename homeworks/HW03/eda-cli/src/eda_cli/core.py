...
def missing_table(df: pd.DataFrame, treshold: float = 0.0) -> pd.DataFrame:
    """
    Таблица пропусков по колонкам: count/share, в которых доля пропусков >= treshold.
    """
    if df.empty:
    ...
    result = result[result["missing_share"] >= treshold]

    result = result.sort_values("missing_share", ascending=False)
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for c in summary.columns:
        if not c.is_numeric and c.unique > (summary.n_rows * 0.9):
            high_cardinality_cols.append(c.name)
    flags["has_high_cardinality_categoricals"] = len(high_cardinality_cols) > 0
    flags["high_cardinality_names"] = high_cardinality_cols

    zero_heavy_cols = []
    for c in summary.columns:
        if c.is_numeric and c.non_null and c.n_zeros is not None:
            zero_share = c.n_zeros / c.non_null
            if zero_share > 0.5: 
                zero_heavy_cols.append(c.name)

    flags["has_many_zero_values"] = len(zero_heavy_cols) > 0
    flags["zero_heavy_cols_list"] = zero_heavy_cols
